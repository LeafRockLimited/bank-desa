                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div class="grid grid-flow-row grid-cols-2 items-end gap-4">
                            <div>
                                ...
                                <InputCurrency :value="form.debit" @update:value="(value) => {form.debit = isNaN(value)? 0 :value}"></InputCurrency>
                            </div>
                            <div>
                            <label for="kredit" class="block text-sm font-medium text-gray-700">Kredit</label>
                                <InputCurrency :value="form.kredit" @update:value="(value) => {form.kredit = isNaN(value)? 0 :value}"></InputCurrency>
                            </div>
                        </div>
...
export default {
    ...
    data() {
        return {
            form: useForm({
                'no_bukti' : this.jurnal.no_bukti,
                'id_rekening' : this.jurnal.id_rekening,
                'debit' : this.jurnal.debit,
                'kredit' : this.jurnal.kredit,
                'keterangan' : this.jurnal.keterangan,
                'tanggal_transaksi' : moment().format('YYYY-MM-DD'),
                'komponen_lak' : this.jurnal.komponen_lak,
            })
        }
    },
    methods: {
        async store() {
            try {
                await axios.put(route('jurnal.update',{ id: this.jurnal.id }), this.form);
